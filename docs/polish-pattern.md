...
- **Hero:** dropped `treemap` (`ChurnTreemap` — repo-wide churn of every file, the RELIC-333 firehose shape). `risk-heatmap` promoted to default, `scatter` retained.
  - **Known gap:** `RiskHeatmap` does not read `report.cursedFiles` either, and is verbatim the `risk` preset's default. It's a closer proxy — it composes the same ownership / blast-radius / shame inputs that feed `curseScore` — but this preset still has no curse-score-native hero. Promoting the existing alt was chosen over building one in this pass. Tracked at [RELIC-343](https://linear.app/nebulord/issue/RELIC-343).
  - **Why the follow-up isn't a quick add:** the obvious hero (stacked contribution-by-reason bars per file) can't be built from the current report. `findCursedFiles` pushes reason strings and adds points as separate statements, and `CursedFile.reasons` is a bare `string[]` — the weights (35 / 30 / 20 / 15 / 10 / 6 / 5) never leave the analyzer, so nothing downstream can attribute score to cause. RELIC-343 is blocked on emitting that attribution, which is a *second* breaking change to `GitrelicReport`.
- **HeroCaption backport:** added to `HotspotScatter` (was missing — backport-style fix paralleling the `BlastHistogram` and `RewriteDivergingBar` captions). It is the `scatter` viz shared by `hotspots` (default), `overview`, `cursed-files` and `dead-code`, so the fix lands in four presets. `RiskHeatmap` already had an equivalent strip, but hand-rolled rather than using the shared component — left alone because converting it means adding an extras slot to `HeroCaption` for its legend row.
